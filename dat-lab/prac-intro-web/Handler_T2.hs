...
newtype Handler a = HandlerC ( ReaderT W.Request (StateT HandlerState IO) a) -- (A completar per l'estudiant)
...
asksRequest f = HandlerC $ ReaderT $ \req -> pure(f req) 

-- Obte informació de l'estat del handler
getsHandlerState :: (HandlerState -> a) -> Handler a
getsHandlerState f =  HandlerC $ ReaderT $ \req -> StateT $ \s -> pure((f s),s)

-- Modifica l'estat del handler
modifyHandlerState :: (HandlerState -> HandlerState) -> Handler ()
modifyHandlerState f = HandlerC $ ReaderT $ \req -> StateT $ \s0 -> pure((),(f s0))
...
lookupPostParams name = do
    ...
    case mbparams of
        Just params -> -- params es una llista de parelles de tipus (Text, Text)
            ...
            let 
                paramMatching = filter (\paramName -> fst(paramName) == name) params
            
                values = map (\param -> snd(param)) paramMatching
            in
                pure values
        Nothing ->
