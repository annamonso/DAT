...
instance Applicative Handler where
    ...
    pure x = HandlerC $ \ req st0 -> pure (x,st0)
        -- (A completar per l'estudiant)
        
    HandlerC hf <*> HandlerC hx = HandlerC $ \req st0 ->
        do 
            (x,st1) <- hf req st0
            (res,st2) <- hx req st1
            pure(x res,st2)
...
-- Obtiene información del estado del handler
getsHandlerState :: (HandlerState -> a) -> Handler a
getsHandlerState f = HandlerC $ \req st0 ->
    pure (f st0, st0)

-- Modifica el estado del handler
modifyHandlerState :: (HandlerState -> HandlerState) -> Handler ()
modifyHandlerState f = HandlerC $ \req st0 ->
    pure ((), f st0)
...
lookupPostParams name = do
    ...
    case mbparams of
        Just params -> -- params es una llista de parelles de tipus (Text, Text)
            ...
            --   filter :: (a -> Bool) -> [a] -> [a]
            -- (A completar per l'estudiant)
            -- Filtra les parelles que tenen el nom correcte
